...
**공유 규칙의 원본은 항상 `~/claude-config/`다.** 거기를 고쳐야 다른 기계에 반영된다.

⚠️ **`~/.claude/CLAUDE.md`를 stub 으로 취급하지 말 것.** 한 줄짜리 import 만 있는 기계도
있지만, 어떤 기계에는 머신 사양·디스크 여유·데이터 취급 원칙·위임 규칙이 들어 있다.
stub 으로 간주하고 덮어쓰면 그 내용이 날아간다(53행이 같은 위험을 이미 경고한다).

**역할 경계**

| 파일 | 담는 것 |
|---|---|
| `~/claude-config/rules/*.md` | **어느 기계에서나 쓰는 방법·판단 기준** (git 동기화) |
| `~/.claude/CLAUDE.md` | **그 기계의 상태·제약** (로컬, 동기화 안 됨) |
| 프로젝트 `CLAUDE.md` | **그 프로젝트의 성질·절차** (프로젝트 저장소) |

머신 파일에는 **사실만 적고 방법은 claude-config 를 가리킨다.**
예: "iCloud 저장공간 최적화 On. 판별법은 `rules/workflow.md` 조사 규율 참조".
경계가 문서화돼 있지 않아, 어느 기계에서나 쓸 지식을 머신 파일에 넣었다가 다시 옮긴
왕복이 실제로 있었다.
