...
- 발견한 것은 `~/claude-config/_inbox.md`에 **끝에 덧붙이기만** 한다
- `rules/` 아래 파일은 **승인된 것만** 들어가는 확정본이다. 승격은 한 번에 **한 세션만** 한다
- 형식·승격 절차는 `_inbox.md` 안에 적혀 있다. 기각 이력도 남긴다(안 남기면 다음 세션이 다시 올린다)

⚠️ **`_inbox.md`는 git이 추적하지 않는다 — 기계 사이로 전파되지 않는다** (2026-08-13 확인).
`.gitignore`가 화이트리스트라 이 파일은 `*`에 걸리고, 그 파일 머리말이 이유를 적어 두었다:
**블랙리스트 시절 이 파일이 그대로 추적돼 공개 저장소로 나갔다.** 이력에서도 지워졌다
(`git log --all -- _inbox.md`가 비어 있다).

- 종전에 이 절은 *"append-only는 자동 병합된다"* 고 적고 있었으나 **사실이 아니다.**
  git으로 가지 않으므로 병합될 것도 없다. 후보는 **그 기계에만** 쌓인다.
- 그러므로 **후보를 올린 기계에서 승격까지 끝내는 것이 안전하다.** 다른 기계에서
  "inbox에 넣어 뒀다"는 말을 믿고 찾으면 없다.
- 화이트리스트를 뚫어 다시 추적하는 것은 **유출 전력이 있는 파일을 되살리는 일**이다.
  하려면 내용에 식별 정보가 없음을 먼저 확인하고, 사용자 승인을 받는다.
